...
def create_rdf_graph_from_json_ld_dict(json_ld_dict: Dict, default_base_ns_str: Optional[str]=None) -> rdflib.Graph:
    ...
    for entity_data in entities_to_process:
        ...
        # Ensure @type is processed correctly
        type_info = entity_data.get("@type")
        if isinstance(type_info, str):
            g.add((subject_uri, RDF.type, expand_uri(type_info)))
        elif isinstance(type_info, list):
            for type_val_str in type_info:
                if isinstance(type_val_str, str):
                    g.add((subject_uri, RDF.type, expand_uri(type_val_str)))
                else:
                    kce_logger.warning(f"Skipping non-string @type value in list: {type_val_str} for subject {subject_uri}")
        elif type_info is not None: # It's not a string or a list, but it's present
             kce_logger.warning(f"Unexpected @type value: {type_info} for subject {subject_uri}. Expected string or list of strings.")
    ...
    # Debugging log: Check if the crucial type triple was added
    problem_instance_type_uri = expand_uri("kce:ProblemInstance")
    found_type_triple = False
    for s, p, o in g.triples((None, RDF.type, problem_instance_type_uri)):
        found_type_triple = True
        break
    if not found_type_triple and any(e.get('@type') == "kce:ProblemInstance" for e in entities_to_process):
        kce_logger.error(f"CRITICAL: kce:ProblemInstance type triple NOT found in graph generated by create_rdf_graph_from_json_ld_dict. Graph len: {len(g)}")
    elif any(e.get('@type') == "kce:ProblemInstance" for e in entities_to_process):
        kce_logger.info(f"CONFIRMED: kce:ProblemInstance type triple FOUND in graph by create_rdf_graph_from_json_ld_dict. Graph len: {len(g)}")
